...
public class _01_GetAllRowColumn extends JDBCParent {
    ...
    public  void  test2() throws  SQLException {
        ...
        ResultSetMetaData rsmd=rs.getMetaData(); //veri harici diger bilgiler

        int columnCount= rsmd.getColumnCount();

        rs.next();// ilk satira gittim
        for (int i = 0; i <= columnCount ; i++) {// kac tane kolon varsa hepsini bilgileri yazdirabilirim
    ...
    public  List<List<String>> getListData(String sprgu){
        List<List<String>> tablo=new ArrayList<>();

        try {
            ResultSet rs = sorguEkrani.executeQuery(sprgu);
            ResultSetMetaData rsmd= rs.getMetaData();
